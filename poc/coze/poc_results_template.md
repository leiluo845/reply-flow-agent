# ReplyFlow Coze POC：Results Template（结果记录模板）

> 本文件是运行记录模板，不是预填成功结果。只有在 Coze 实际运行后填写，不能用手工编写的“看起来合理”的输出替代。

## 1. 运行环境

| 项 | 填写内容 |
|---|---|
| Coze 区域/工作区 | 国内 `coze.cn` / 工作区名称 |
...
| 工作流导出路径 | `poc/coze/ReplyFlow_POC_export.*` 或“不支持/未导出” |

## 2. 每条案例记录

| Case ID | task_type | Coze Run/Execute ID | 输入摘要 | 原始输出是否保存 | Schema 解析 | 6项评分 | 结果（通过/失败） | 失败原因/修正动作 |
|---|---|---|---|---|---|---|---|---|
...
如果输出包含个人信息、真实订单、Coze PAT/Token 或其他敏感内容，立即删除并重新使用虚构数据；Coze PAT/Token 永远不得写入此文件。
...
- [ ] 8 条案例都有实际 Coze Run/Execute ID 或可追溯的工作台运行记录；
