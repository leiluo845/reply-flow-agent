...
2026-08-27 风险分级修正：阶段 B 预置邮件邮箱现与其关联订单买家邮箱一致；选择订单但保留默认模拟邮箱时，服务端自动使用订单买家邮箱。收紧“已送达但未收到”的事实冲突规则，避免“未收到退货标签”被误判为缺件。真实 Coze 批处理复核结果为 6 封：`L1=1`、`L2=3`、`L3=2`、`失败=0`；全量测试 `112 passed`。

2026-08-31 评测复现：重新运行 `evals/run_eval.py --mode demo`，30 条案例的指标与历史报告完全一致：13 条 R2 中正确识别 8 条，`high_risk_recall=8/13=61.5%`，结论仍为 `No-Go`。R2 漏判仍是 CASE-014、015、016、018、028；本次仅刷新逐案例 Trace 编号。`tests/test_eval_metrics.py` 通过。

2026-09-03 Interactive 复测：Coze 工作区额度可用，重新运行 `evals/run_eval.py --mode interactive` 完成 30 条案例。真实结果为意图准确率 `18/28=64.3%`、订单号准确率 `22/25=88.0%`、动态接入率 `29/29=100%`、高风险召回 `8/13=61.5%`、未授权承诺违规 `1`、无依据订单事实违规 `0`；4 条案例因 Coze 输出未通过本地结构校验（`MODEL_OUTPUT_INVALID`）失败，自动结论仍为 `No-Go`。本次 No-Go 依据是模型输出和本地安全门槛，不是额度不足。

2026-08-28 投递材料补充：新增 `docs/ReplyFlow_项目作品集制作指南.md`，明确 PDF 主附件、HTML 在线案例页、视频/GitHub 补充材料的分工，并提供页面结构、面试叙事、动态演示、评测披露和投递前自查清单。
...
- 结果（2026-09-03 复测）：Coze 额度可用，30 条案例完成评测；意图准确率 64.3%（18/28）、订单号准确率 88.0%（22/25）、动态接入率 100%（29/29）、高风险召回 61.5%（8/13）、未授权承诺违规 1、无依据订单事实违规 0，自动判定 `No-Go`。4 条案例因 `MODEL_OUTPUT_INVALID` 失败，详细结果和 trace_ref 见 `evals/reports/eval_interactive.md` 和 `.json`。
...
- 阶段 14 已完成；Interactive 已于 2026-09-03 在 Coze 额度可用时复测，保留真实模型失败和安全门槛结果；下一步是修复结构化输出和高风险召回，不降低案例标注或安全门槛。
