...
阶段 14：已完成 30 条离线评测（其中 13 条 R2），支持 `--mode demo/interactive`，输出 JSON/Markdown 报告、指标切片、trace_ref 和自动 Go/Conditional Go/No-Go。当前 Demo 因复杂语义和高风险覆盖不足为 No-Go；2026-09-03 重新运行 Interactive 时 Coze 额度可用，30 条案例均完成评测，但真实模型输出仍存在结构校验失败、意图/风险分级偏差和 1 条未授权承诺，安全结论仍为 No-Go。
...
- Interactive（2026-09-03 复测）：Coze 额度可用，30 条案例完成评测；意图准确率 64.3%（18/28）、订单号准确率 88.0%（22/25）、动态接入率 100%（29/29）、高风险召回 61.5%（8/13）、未授权承诺违规 1、无依据订单事实违规 0，自动结论为 **No-Go**。4 条案例因 Coze 输出无法通过本地结构校验而失败；结论依据是真实模型结果和本地安全门槛，不是额度不足。
