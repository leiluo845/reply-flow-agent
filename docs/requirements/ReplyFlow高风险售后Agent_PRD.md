...
7. 提供页面底部固定的“模拟邮件台”浮动按钮；点击后打开浮窗，允许用户输入一行邮件正文、选择虚构订单并提交，让邮件真实写入本地收件箱并进入顶部聚合站内信，再由 Agent 处理。
8. 页面不提供 Demo Mode / Interactive Mode 切换，只通过“智能客服”开关控制 Coze 是否运行；关闭时不调用 AI，开启时调用 Coze，Coze 失败直接提示错误且不回退本地规则。
...
- 位置：页面底部固定“模拟邮件台”浮动按钮，点击后打开模态浮窗；提交成功后浮窗关闭。
- 默认状态：关闭，避免干扰正式工作台。
- 按钮文案：`模拟邮件台`；浮窗标题：`模拟收到邮件`。
...
| 智能客服开关 | 是 | 位于邮件详情区，默认关闭；开启后提交邮件调用 Coze |
...
| 按钮 | 行为 |
|---|---|
| 模拟收到邮件 | 校验正文，写入原始收件箱，识别买家站内信，创建聚合会话；智能客服开启时继续执行 Agent |
| 清空输入 | 清除当前浮窗输入，不影响数据库 |
| 重试 AI | 仅在 AI 处理失败时展示；智能客服开启后重新调用 Coze，复用同一邮件和订单上下文，不重复写入邮件 |
...
- 智能客服关闭：不调用模型，邮件停留在待分析；
- 智能客服开启：使用 Coze 分析自由文本，但事实、风险、发送和审计仍由本地控制层负责；
- Coze 调用失败：提示 AI 处理失败并保留失败状态，不回退本地规则、不伪造草稿。
...
文件夹栏只作为原型视觉占位，不支持切换；点击“模拟收到邮件”后，原始收件箱和顶部聚合站内信计数仍会真实变化，新会话自动选中。
...
筛选、搜索、日期、分页和文件夹切换均只做视觉占位，不实现交互；不新增“政策邮件”或“政策文件夹”筛选。
...
| 级别 | 主按钮 | 行为 |
|---|---|---|
| 一级 | 系统自动处理 | 页面展示自动处理结果、风险等级和 AI 回复标识，不需要店管点击 AI回复 |
| 二级 | AI回复 | 生成草稿并写入输入框，店管可编辑和模拟发送 |
| 三级 | 生成参考回复 | 生成草稿并写入输入框，同时显示高风险标识和核对清单 |
...
页面只保留一个“智能客服”开关，不展示 Demo Mode / Interactive Mode 切换。

### 12.1 开关关闭

- 邮件仍可通过模拟邮件台接收并进入聚合站内信；
- 不调用 Coze，不生成草稿，不自动发送；
- 当前会话保持“待分析”状态，供面试官理解 Agent 未介入时的原始工作台。
...
- Coze 不可用时显示“AI 处理失败”和错误码，不切换本地规则、不伪造草稿。
...
flowchart TB
    ...
    UI --> SWITCH{智能客服开关}
    SWITCH -->|关闭| WAIT[保持待分析，不调用 AI]
    SWITCH -->|开启| COZE[Coze Workflow]
    COZE --> ORCH
...
| M5 | Coze 单模式、确认、幂等、审计和评测 | 7-10 小时 |
| M6 | 作品集、视频和面试演示 | 5-7 小时 |

### 18.1 MVP 验收清单

- [ ] 新对话中的 AI 只读本 PRD 和行动指南，就能知道取消项。
- [ ] 页面底部“模拟邮件台”能打开模拟邮件浮窗；选择订单后即时显示订单摘要，并能输入一行邮件写入本地收件箱。
...
- [ ] 所有写操作有确认、幂等和审计。
- [ ] 智能客服关闭时不调用 Coze；开启后 Coze 失败显示错误，不回退本地规则。
...
2. **一级邮件（60 秒）**：打开智能客服开关，点击页面底部“模拟邮件台”，选择 `ORD-1001` 查看订单摘要，输入 `Hi, where is order ORD-1001?` 并提交；展示收件箱和站内信计数变化、新会话置顶、订单/物流 Tool Trace 和“AI已回复”。
...
演示过程中必须明确说明：智能客服关闭时只是原邮件工作台；打开后才调用 Coze。现场 Coze API 不稳定时，页面显示 AI 处理失败，不伪造模型结果。
...
| 智能客服开关 | 控制是否调用 Coze；关闭时只展示原邮件工作台 |
