...
“演示控制台（模拟邮件接入）”是作品演示工具，不属于正式店管业务菜单。它模拟真实邮件同步入口，用于向面试官展示邮件如何从输入进入收件箱、聚合站内信和 Agent 流程。

### 8.2 入口与默认状态

- 位置：顶部聚合站内信页面顶部或右上角，可折叠。
- 默认状态：折叠，避免干扰正式工作台。
- 标题：`演示控制台（模拟邮件接入）`。
...
2. **一级邮件（60 秒）**：展开演示控制台，输入 `Hi, where is order ORD-1001?`，点击“模拟收到邮件”；展示收件箱和站内信计数变化、新会话置顶、订单/物流 Tool Trace 和“AI已回复”。
