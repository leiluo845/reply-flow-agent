...
动态演示从折叠的“演示控制台（模拟邮件接入）”开始：用户可以输入一行英文邮件，点击“模拟收到邮件”，系统将它真实写入 SQLite 原始收件箱，创建顶部聚合会话，然后由单 Agent 分析、查询订单和物流、检索内部回复依据、判断处理级别并生成或执行模拟回复。
...
先用不超过10行复述：产品搭载位置、唯一用户、三级处理、演示控制台、取消项和本轮范围。
...
本轮只做阶段1，不写Python代码。
创建：
- docs/product_contract.md：完整记录搭载位置、店管单角色、三级处理、演示控制台、取消项；
...
```text
本轮只做Streamlit顶部聚合站内信和动态演示控制台，不改核心规则。
...
控制台字段：正文必填、主题可选、模拟发件人可选、关联模拟订单可选、运行模式、接收后自动运行。
...
1. 展开控制台，输入 `Hi, where is order ORD-1001?`；
2. 点击“模拟收到邮件”；
3. 确认收件箱计数和顶部站内信计数增加；
...
2. **60秒**：展开演示控制台，输入一级物流邮件，点击“模拟收到邮件”。
