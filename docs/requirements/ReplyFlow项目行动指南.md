...
动态演示从页面右下角的“模拟收到邮件”浮动按钮开始：点击后打开浮窗，用户可以输入一行英文邮件并选择虚构订单查看摘要，再点击“模拟收到邮件”；系统将它真实写入 SQLite 原始收件箱，创建顶部聚合会话，然后由单 Agent 分析、查询订单和物流、检索内部回复依据、判断处理级别并生成或执行模拟回复。
...
```text
本轮只做 Streamlit 顶部聚合站内信和动态模拟邮件浮窗，不改核心规则。
...
浮窗字段：正文必填、主题可选、模拟发件人可选、关联模拟订单可选；订单候选展示订单号、商品、金额、履约状态，选中后即时展示订单摘要；运行模式和接收后自动运行沿用侧栏设置。
按钮：模拟收到邮件、仅接收不处理、清空输入、重置演示数据（二次确认）。
...
1. 点击右下角“模拟收到邮件”，选择 `ORD-1001` 查看订单摘要，输入 `Hi, where is order ORD-1001?`；
2. 点击“模拟收到邮件”；
3. 确认收件箱计数和顶部站内信计数增加；
...
**当前验收记录（2026-08-24）**：已完成 `app.py` 动态工作台和 `tests/test_ui_helpers.py`。浏览器访问 `http://localhost:8506` 已验证：页面采用邮箱工作台分栏布局，右下角浮动按钮可打开模拟邮件浮窗，订单选择后即时展示订单摘要；提交后邮件进入原始收件箱和顶部聚合，自动选中新会话并在右侧展示关联订单。一级自动回复并写入本地 outbox、二级出现可编辑草稿并需确认、三级未完成清单时发送按钮禁用、清单全选后可模拟发送、重复点击不增加 outbox。Interactive Mode 与 Demo Mode 共用页面，Coze 失败时不伪造草稿。
