...
- 尚未安装业务框架，尚未编写业务代码。
- 当前阶段可以先继续本地开发：`poc/coze/` 已完成离线契约；实际登录扣子、创建 Workflow 和运行 8 条案例可后置到阶段 11 前后。登录前不得伪造 Run/Execute ID 或 POC 通过结论。
...
当前阶段是阶段 2：Coze 最小 POC 离线契约已准备。先检查 `poc/coze/` 和 PROJECT_STATUS；如果没有真实 Coze 运行记录，可以继续进入阶段 3 做本地 Demo，但必须保留 Coze 的接口边界和 Mock 适配，不要伪造 Coze 结果。
```
