...
- 当前下一步是阶段 11：Coze Interactive Mode。需要在真实创建/发布 Workflow 时再登录 Coze；未登录前不得伪造 POC 记录。
...
当前阶段是阶段 11：Coze Interactive Mode。阶段 3 工程骨架、阶段 4 虚构种子数据、阶段 5 SQLite 数据层、阶段 6 模拟接入/聚合、阶段 7 MCP Tools、阶段 8 Skills/依据检索、阶段 9 风险网关和阶段 10 Demo Mode 均已完成；先运行 `python -m pytest -q` 确认基线通过，再按行动指南阶段 11 实现 Coze 客户端和 mock 测试。只有需要创建/发布真实 Workflow 时才让用户登录 Coze，不要登录真实 Amazon，不要连接真实邮箱，不要伪造 Coze 运行记录。
```
