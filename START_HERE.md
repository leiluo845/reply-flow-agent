...
- 当前下一步是阶段 5：SQLite 数据层和聚合模型。阶段 5 不需要登录 Coze。
...
当前阶段是阶段 5：SQLite 数据层和聚合模型。阶段 3 工程骨架和阶段 4 虚构种子数据均已完成；先运行 `python -m pytest -q` 确认基线通过，再按行动指南阶段 5 创建表结构、Pydantic 模型和聚合逻辑。不要登录真实 Amazon，不要连接真实邮箱，不要伪造 Coze 运行记录。
```
