...
- 当前下一步是阶段 10：Demo Mode 状态机与有限规则编排。阶段 10 不需要登录 Coze。
...
当前阶段是阶段 10：Demo Mode 状态机与有限规则编排。阶段 3 工程骨架、阶段 4 虚构种子数据、阶段 5 SQLite 数据层、阶段 6 模拟接入/聚合、阶段 7 MCP Tools、阶段 8 Skills/依据检索和阶段 9 风险网关均已完成；先运行 `python -m pytest -q` 确认基线通过，再按行动指南阶段 10 实现状态机、Demo Router 和 Orchestrator。不要登录真实 Amazon，不要连接真实邮箱，不要伪造 Coze 运行记录。
```
