...
- 当前下一步是阶段 4：创建全虚构种子数据和只读回复依据。阶段 4 不需要登录 Coze。
...
当前阶段是阶段 4：虚构种子数据和只读回复依据。阶段 3 工程骨架已完成；先运行 `python -m pytest -q` 确认基线通过，再按行动指南阶段 4 创建全虚构邮件、订单、物流和回复依据。不要登录真实 Amazon，不要连接真实邮箱，不要伪造 Coze 运行记录。
```
